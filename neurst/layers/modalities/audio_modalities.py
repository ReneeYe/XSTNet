...
class Conv1dSubsampling(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        """
        self._params = extract_constructor_params(locals(), verbose=True)
        super(Conv1dSubsampling, self).__init__(name=name)
        self._channels = channels
        self._strides = list(strides)
        self._kernel_sizes = list(kernel_sizes)
        self._layer_num = len(kernel_sizes)
        self._layer_dropout = layer_dropout
        self._layer_norm = layer_norm
        self._stacking_conv_layers = []
    ...
    def build(self, input_shape):
        ...
        for idx in range(self._layer_num):
            self._stacking_conv_layers.append([
                tf.keras.layers.Conv1D(
                    filters=self._channels,
                    kernel_size=self._kernel_sizes[idx],
                    strides=self._strides[idx],
                    padding="VALID",
                    activation=None,
                    name=f"conv{idx}"),

                tf.keras.layers.LayerNormalization(
                    epsilon=1e-6, dtype="float32", name=f"ln{idx}") if self._layer_norm
                else None,

                tf.keras.layers.Dropout(rate=self._layer_dropout) if self._layer_dropout > 0
                else None
            ])

        super(Conv1dSubsampling, self).__init__(input_shape)

    def call(self, inputs, **kwargs):
        """
         Args:
            inputs: An float tensor with shape [batch_size, length, feature_dim].
        """
        x = inputs
        for i, conv_layer in enumerate(self._stacking_conv_layers):
            conv, layer_norm, drop_layer = conv_layer
            manual_padding = [[0, 0], [self._kernel_sizes[i] // 2, self._kernel_sizes[i] // 2], [0, 0]]
            x = conv(tf.pad(x, manual_padding))
            if layer_norm:
                x = layer_norm(x)
            x = gelu(x, non_approximate=True)
            if drop_layer:
                x = drop_layer(x, training=kwargs.get("is_training", False))
