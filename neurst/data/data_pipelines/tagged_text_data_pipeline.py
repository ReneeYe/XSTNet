...
@register_data_pipeline("tagged_text")
class TaggedTextDataPipeline(DataPipeline, Vocab):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """ Initializes the data pipeline for text data with language indicator at the begining.
        ...
        if self._subtokenizer is not None:
            if subtokenizer_codes is None:
                ...
                self._glossary_dict = self._subtokenizer._glossary_dict
    ...
    def meta(self):
        return {
            ...
            "tag_dict": self._glossary_dict
        }
    
    def set_language(self, lang):
        self._language = lang

    def lang2idx(self, input):
        """
        Args:
            input: A token, eg. '<en>', '<de>', etc.
        Returns:
            the token id of language input
        """
        return Vocab.map_token_to_id(self, input, unknown_default=self._unk_id)
    ...
    def process(self, input, is_processed=False):
        ...
        if isinstance(input, str) and input.startswith("<"):
            tag = input.split()[0]
            untag_input = " ".join(input.split()[1:])
        else:
            tag = f'<{self._language}>'
        if not is_processed:
            if self._tokenizer:
                untag_input = self._tokenizer.tokenize(untag_input)
            if self._subtokenizer:
                untag_input = self._subtokenizer.tokenize(untag_input, return_str=False)
        if isinstance(untag_input, str):
            untag_input = untag_input.split()
        input = untag_input if tag is None else [tag] + untag_input
    ...
    def recover(self, input):
        ...
        if (input[0] == self._bos_id) or (input[0] in self._glossary_dict.values()):
            input = input[1:]
        try:
