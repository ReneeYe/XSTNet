...
V1_0_LANGS="de es fr it nl pt ro ru"
V1_2_LANGS="ar cs fa tr vi zh"

if [[ "$V1_0_LANGS" =~ "$TRG_LANGUAGE" ]]; then
    INPUT_TARBALL="MUSTC_v1.0_en-${TRG_LANGUAGE}.tar.gz"
elif [[ "$V1_2_LANGS" =~ "$TRG_LANGUAGE" ]]; then
    INPUT_TARBALL="MUSTC_v1.2_en-${TRG_LANGUAGE}.tar.gz"
else
    echo "Unsupported language: "$TRG_LANGUAGE
    exit 1
fi

if [[ $(fileExists $RAW_DATA_PATH $INPUT_TARBALL) -eq 0 ]]; then
    echo "File not exists: $RAW_DATA_PATH/$INPUT_TARBALL"
    echo "Please download and save it to $RAW_DATA_PATH in advance"
    exit 1
fi

INPUT_TARBALL=$RAW_DATA_PATH/$INPUT_TARBALL
